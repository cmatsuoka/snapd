...
func Run(gadgetRoot, device string, options Options, observer gadget.ContentObserver) error {
	...
	loadChain := []secboot.EFIImage{
		...
		secboot.FileEFIImage(filepath.Join(boot.InitramfsUbuntuSeedDir, "EFI/boot/bootx64.efi")),
		// the path to the recovery grub EFI binary
		secboot.FileEFIImage(filepath.Join(boot.InitramfsUbuntuSeedDir, "EFI/boot/grubx64.efi")),
		// the path to the run mode grub EFI binary
		secboot.FileEFIImage(filepath.Join(boot.InitramfsUbuntuBootDir, "EFI/boot/grubx64.efi")),
	}
	if options.KernelPath != "" {
		// the path to the kernel EFI binary
		loadChain = append(loadChain, secboot.FileEFIImage(options.KernelPath))
	}
